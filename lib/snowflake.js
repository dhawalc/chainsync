...
export function initializeSnowflake() {
  try {
    // Check if password authentication is available (for Docker environment)
    if (process.env.SNOWFLAKE_PASSWORD) {
      console.log('Using password authentication for Snowflake.');
    ...
    // Fall back to JWT authentication if password is not available
    let privateKey;
    ...
    console.log('Snowflake connection initialized with JWT auth');
    return connection;
  } catch (error) {
